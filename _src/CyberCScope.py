...
class CyberCScope(object):
    ...
    def model_compressinon(self, cur_n, X):
        ...
        if self.anomaly:
            ...
            #         self.all_comp_regime.counterA
            #     ) + self.counterA
            #     self.all_comp_regime.suffstatsMc = (
            #         self.all_comp_regime.suffstatsMc
            #     ) + self.suffstatsMc
...
def _gibbs_sampling_online(
    ...
    for p in range(l):
        ...
        for mode_ in range(n_continuous_modes):
            for i in range(k):
                prev_terms[mode_][:, i] += (
                    prev_distributions[mode_][p][:, i]/l
                )
            # print(suffstatsMc)
            # suffstatsMc[mode_][:5] = prev_terms[mode_][:5]
...
# def _compute_gm_llh(X_continuous, gm_factors):
#     cost = 0
#     N = X_continuous.shape[0]
#     for x, gm_factor in zip(X_continuous.T, gm_factors):
#         S = np.sum(x)
#         lnS = np.sum(np.log(ZERO+x))
#         for shape, scale in gm_factor.T:
#             cost -= llh_gamma(N, S, lnS, shape, scale)
#     # transform base    
#     return cost/np.log(2.0)
...
#         x += 1
#     z = 1.0 / (x * x)
#     result =  result + np.log(x) - 0.5 / x - z * (1.0 / 12.0 - z * (1.0 / 120.0))
#     return result
# @numba.jit(nopython=True)
# def approx_digamma(x):
#     eps = 1e-10
#     gamma = 0.57721566490153286060  # eular constant
#     x += eps
...
#         x += 1
#     x -= 1
#     result += -gamma - 1 / x + (x - 1) / (2 * x**2) - (x - 1)**2 / (6 * x**3)
#     return result


# @numba.jit(nopython=True)
# def gamma_function(x,eps=1e-10):
#     x += eps
#     if x < 0.5:
#         return np.pi / (np.sin(np.pi * x) * gamma_function(1 - x))
#     else:
#         x -= 1
#         return np.sqrt(2 * np.pi) * (x**x) * np.exp(-x) * (1 + 1 / (12 * x) + 1 / (288 * x**2))
...
#     #     return 0.0
#     coefficient = (beta**alpha) / gamma(alpha)
#     return coefficient * (x**(alpha - 1)) * np.exp(-beta * x)
